...
function mapToRuntimeEvents(
	...
	if (event.method === "turn/failed") {
		const message =
			asString(payload?.message) ?? event.message ?? "Turn failed";
		return [
			{
				...runtimeEventBase(event, canonicalThreadId),
				type: "runtime.error",
	...
	if (event.method === "process/stderr") {
		const message =
			asString(payload?.message) ?? event.message ?? "Stderr output";
		return [
			{
				...runtimeEventBase(event, canonicalThreadId),
		...
		const manager = yield* Effect.acquireRelease(
			Effect.gen(function* () {
				...
				return (
					...
					new ClaudeCodeAppServerManager(services)
				);
			}),
			(manager) =>
				Effect.sync(() => {
			...
			return Effect.tryPromise({
				try: () => manager.startSession(managerInput),
					...
					new ProviderAdapterProcessError({
						...
						detail: toMessage(
							cause,
							"Failed to start Claude Code adapter session.",
						),
						cause,
					}),
			});
					...
					Effect.gen(function* () {
						...
						if (runtimeEvents.length === 0) {
							yield* Effect.logDebug(
								"ignoring unhandled Claude Code provider event",
								{
									method: event.method,
