...
describe("claudeCodeAppServerHelpers", () => {
	describe("normalizeClaudeCodeModelSlug", () => {
		...
		it("returns trimmed slug for non-empty input", () => {
			expect(normalizeClaudeCodeModelSlug("claude-sonnet-4-6")).toBe(
				"claude-sonnet-4-6",
			);
			expect(normalizeClaudeCodeModelSlug("  claude-opus  ")).toBe(
				"claude-opus",
			);
		});
	});
	...
	describe("buildPermissionFlags", () => {
		it("returns --dangerously-skip-permissions for full-access", () => {
			expect(buildPermissionFlags("full-access")).toEqual([
	...
	describe("detailFromToolInput", () => {
		it("returns command for Bash input", () => {
			...
			expect(detailFromToolInput("Bash", { command: "  echo hi  " })).toBe(
				"echo hi",
			);
		...
		it("returns path for file tools (file_path or path)", () => {
			expect(detailFromToolInput("Read", { file_path: "/tmp/foo" })).toBe(
				"/tmp/foo",
			);
			expect(detailFromToolInput("Write", { path: "src/index.ts" })).toBe(
				"src/index.ts",
			);
	...
	describe("pathFromToolInput", () => {
		it("returns path for file tools", () => {
			expect(pathFromToolInput("Read", { file_path: "/tmp/foo" })).toBe(
				"/tmp/foo",
			);
	...
	describe("classifyClaudeCodeStderrLine", () => {
		...
		it("strips ANSI escapes and returns message", () => {
			expect(
				classifyClaudeCodeStderrLine("\x1b[31mError: something\x1b[0m"),
			).toEqual({ message: "Error: something" });
		});
		...
		it("returns null for benign patterns", () => {
			...
			expect(
				classifyClaudeCodeStderrLine("   at Object.<anonymous>"),
			).toBeNull();
		});
